...
function(input, output, session) {
  ...
  output$season_eff_plot <- renderPlotly({
    ...
    print(glue("[{sprintf('%.7f', round(Sys.time() - begin, 7))}] efficiency plot generated."))
  ...
  output$season_scatter <- renderPlotly({
    ...
    print(glue("[{sprintf('%.7f', round(Sys.time() - begin, 7))}] season scatter plot generated."))
  ...
  output$player_news <- renderDT({
    ...
    print(glue("[{sprintf('%.7f', round(Sys.time() - begin, 7))}] player news generated."))
  ...
  output$plot_season <- renderPlotly({
    ...
    plot_df <- dfs %>%
      ...
      filter(G > 10) %>%
      mutate(var = .[[input$plot_season_var]])

    if (input$plot_season_36 &&
        (input$plot_season_var == 'GMSC' || str_detect(input$plot_season_var, 'PG$'))) {
      plot_df <- plot_df %>%
        mutate(var = var / MPG * 36)
    }

    plot_df_selected <- filter(plot_df, PLAYER == input$name)
    ...
    p <- plot_df %>%
      ...
      add_annotations(
        ...
        ax = plot_df_selected$SEASON[-nrow(plot_df_selected)],
        ay = plot_df_selected[[input$plot_season_var]][-nrow(plot_df_selected)],
        x = plot_df_selected$SEASON[-1],
        y = plot_df_selected[[input$plot_season_var]][-1],
        xref = 'x',
        yref = 'y',
        axref = 'x',
        ayref = 'y',
    ...
    print(glue("[{sprintf('%.7f', round(Sys.time() - begin, 7))}] player box-and-whisker plot generated."))
  ...
  output$lab_team_season <- renderPlotly({
    ...
    dfs_everything %>%
      ...
      #summarize_team() %>%
      ungroup() %>%
      mutate(var = .[[input$lab_team_season_var]]) %>%
  ...
  output$lab_player_season <- renderPlotly({
    ...
    dfs_everything %>%
      ...
      summarize_per_game(formatting = FALSE) %>%
      ungroup() %>%
      mutate(var = .[[input$lab_player_season_var]]) %>%
