#' Get raw statlines from team sheets
#'
#' @param delete_before Date string, remove any games prior to this (for if we 
#'   have previous season stats still in team sheets)
#'
#' @return a stacked data frame of all statlines for this season
#' @export
...
import_team_sheet <- function(myurl, team) {
  if (myurl != "") {
    message(glue::glue("Downloading data from {team}..."))
    read_csv(myurl,
             col_types = cols(
               DATE = col_character(),
               OPP = col_character(),
      ...
      select(TEAM, DATE, OPP, PLAYER, M, P, R, OR, DR, A, S, B, TO, 
...
check_allstats <- function(allstats) {
  ...
  if (nrow(bad_minute_games) > 0) {
    View(bad_minute_games)
    warning(glue::glue("There are {nrow(bad_minute_games)} game(s) where the total minutes doesn't make sense."))
  }
  ...
  if (nrow(bad_sanity_checks) > 0) {
    View(bad_sanity_checks)
    warning(glue::glue("There are {nrow(bad_sanity_checks)} row(s) where the numbers don't make sense."))
  }
  ...
  if (nrow(bad_missing) > 0) {
    View(bad_missing)
    warning(glue::glue("There are {nrow(bad_missing)} row(s) with missing data."))
  }
...
# Export functions ----
# Export to disc and load to sheets
upload_sheet <- function(df, sheetname, newsheet = FALSE) {
  df %>% openxlsx::write.xlsx(export_dir("tmp.xlsx"))
  
  if (newsheet) {
    agg_gs4 <- googledrive::drive_upload(
      export_dir("tmp.xlsx"),
      drive_dir(sheetname),
      type = "spreadsheet",
      overwrite = TRUE
...
upload_plot <- function(p, plotname, 
